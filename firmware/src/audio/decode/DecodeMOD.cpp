...
namespace audio
{
    ...
    bool DecodeMOD::loop()
    {
        if (!_run) goto done; // Easy-peasy

        // First, try and push in the stored sample.  If we can't, then punt and try later
        if (!_output->ConsumeSample(_sample)) goto done; // FIFO full, wait...
        ...
        do
        {
            ...
            GetSample(_sample);
            mixerTick--;
        } while (_output->ConsumeSample(_sample));

    done:
        _source->loop();
        _output->loop();

        // We'll be left with one sample still in our 
        // buffer because it couldn't fit in the FIFO
        return _run;
    }

    bool DecodeMOD::stop()
    {
        // We may be stopping because of allocation
        // failures, so always deallocate
        for (int i = 0; i < CHANNELS; i++)
        {
        ...
        if (_run || (_source && _source->isOpen()))
        {
            // flush I2S output buffer, if the 
            // player was actually running before
            _output->flush();
        }
    ...
    bool DecodeMOD::setBufferSize(int sz)
    {
        if (_run || sz < 1 ) return false;
        fatBufferSize = sz;
        return true;
    }

    bool DecodeMOD::setStereoSeparation(int sep)
    {
        if (_run || sep < 0 || sep>64) return false;
        stereoSeparation = sep;
        return true;
    }

    bool DecodeMOD::setPAL(bool use)
    {
    ...
    bool DecodeMOD::LoadHeader()
    {
        uint8_t i;
        uint8_t temp[4];
        uint8_t junk[22];

        if (20 != _source->read(/*Mod.name*/junk, 20)) return false; // Skip MOD name
        for (i = 0; i < SAMPLES; i++) 
        {
            if (22 != _source->read(junk /*Mod.samples[i].name*/, 22)) return false; // Skip sample name
            if (2 != _source->read(temp, 2)) return false;
            Mod.samples[i].length = MakeWord(temp[0], temp[1]) * 2;
            if (1 != _source->read(reinterpret_cast<uint8_t*>(&Mod.samples[i].fineTune), 1)) return false;
            if (Mod.samples[i].fineTune > 7) Mod.samples[i].fineTune -= 16;
            if (1 != _source->read(&Mod.samples[i].volume, 1)) return false;
            if (2 != _source->read(temp, 2)) return false;
        ...
        if (1 != _source->read(&Mod.songLength, 1)) return false;
        if (1 != _source->read(temp, 1)) return false; // Discard this byte

        Mod.numberOfPatterns = 0;
        for (i = 0; i < 128; i++) 
        {
            if (1 != _source->read(&Mod.order[i], 1)) return false;
        ...
        // Offset 1080
        if (4 != _source->read(temp, 4)) return false;;
    ...
    void DecodeMOD::GetSample(int16_t sample[2])
    {
        int32_t sumL;
        int32_t sumR;
        uint8_t channel;
        ...
        for (channel = 0; channel < Mod.numberOfChannels; channel++) 
        {
                ...
                Mixer.channelSampleNumber[channel] != FatBuffer.channelSampleNumber[channel])
            {
                ...
                if (!_source->seek(samplePointer, SEEK_SET)) 
                {
                    stop();
                    return;
                }
                if (toRead != _source->read(FatBuffer.channels[channel], toRead)) 
                {
                    stop();
                    return;
                }
            ...
            // Actually we could even gain more bits, I was just not sure if more bits would cause overflows in other computations.
        ...
        // Downscale to BITDEPTH - a bit faster because the compiler can replaced division
        // by constants with proper "right shift" + correct handling of sign bit
        ...
        sample[Output::LEFTCHANNEL] = sumL;
        sample[Output::RIGHTCHANNEL] = sumR;
    }

    bool DecodeMOD::RunPlayer()
    {
        if (!_run) return false;
    ...
    bool DecodeMOD::LoadPattern(uint8_t pattern)
    {
        uint8_t row;
        uint8_t channel;
        uint8_t i;
        ...
        if (!_source->seek(1084 + pattern * ROWS * Mod.numberOfChannels * 4, SEEK_SET)) return false;
    ...
    void DecodeMOD::Tremolo(uint8_t channel)
    {
        uint16_t delta;
        uint16_t temp;
    ...
    void DecodeMOD::Portamento(uint8_t channel)
    {
    ...
    void DecodeMOD::Vibrato(uint8_t channel)
    {
        uint16_t delta;
        uint16_t temp;
