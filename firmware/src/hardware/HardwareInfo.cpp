...
{
    switch(PowerSource.getType())
    {
        case PowerSourceType::Unknown:
        {    powerSource = String(PowerSource.getInputVoltage()) + "V";
        }    break;

        case PowerSourceType::Battery:
        {   auto batVolts = PowerSource.getInputVoltage();
            auto batLevel = PowerSource.getPatteryLevelPercents();
            powerSource = "BAT " + String(batVolts) + "V / " + String(batLevel) + "%";
        }   break;

        case PowerSourceType::USB:
        {   auto usbVolts = PowerSource.getInputVoltage();
